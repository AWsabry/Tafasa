            ...
            <div key={favorite.id} className="card overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <div className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <h3 className="font-semibold text-lg text-[var(--color-text-primary)]">{favorite.name}</h3>
                </div>
                {favorite.category && (
                  <div className="text-[var(--color-secondary-dark)] text-xs font-medium mb-2">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 btn btn-primary btn-compact text-sm"
                  >
                    View Details
                  </button>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto p-4">
          ...
          <div className="relative bg-[var(--color-bg-secondary)] rounded-lg w-full max-w-3xl p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-[var(--color-bg-tertiary)] pb-4 mb-4">
              <div>
                <h3 className="text-2xl font-bold text-[var(--color-text-primary)]">{selectedMeal.name}</h3>
                {selectedMeal.category && (
                  <span className="inline-block mt-2 px-3 py-1 rounded-md bg-[var(--color-secondary)] bg-opacity-10 text-[var(--color-secondary-dark)] text-sm font-medium">
              ...
              <button
                onClick={closeModal}
                className="rounded-lg p-2 hover:bg-[var(--color-bg-tertiary)] text-[var(--color-text-primary)]"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="space-y-3">
                  <div>
                    <h4 className="font-semibold mb-1 text-[var(--color-text-primary)]">Description</h4>
              ...
              <div className="space-y-4">
                <div>
                  <h4 className="font-semibold mb-3 flex items-center gap-2 text-[var(--color-text-primary)]">
                    ...
                    {selectedMeal.ingredients && selectedMeal.ingredients.length > 0 ? (
                      <ul className="space-y-2">
                        {selectedMeal.ingredients.map((ing, i) => (
                          <li key={i} className="text-sm flex items-start gap-2 text-[var(--color-text-primary)]">
                      ...
                      <div className="space-y-3">
                        {selectedMeal.preparationSteps.map((step) => (
                          <div key={step.step} className="flex gap-3">
                            ...
                            <p className="text-sm flex-1 pt-1 text-[var(--color-text-primary)]">{step.description}</p>
                          </div>
                        ))}
                      </div>
                    ) : (
            ...
            <div className="mt-6 pt-4 border-t border-[var(--color-bg-tertiary)] flex gap-3">
              <button
                ...
                className="flex-1 btn btn-danger disabled:opacity-60"
              >
